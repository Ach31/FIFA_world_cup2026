...
*Automatisation de la récupération, du traitement et de la visualisation des données des joueurs de la Coupe du Monde 2026 avec **Apache Airflow**, **Docker**, **Kaggle** et **Power BI**.*
...
Apache Airflow 2.8.1 (pour l'automatisation des workflows).

Backend :

Python 3.9+ (pour les scripts de traitement).
Pandas (pour la manipulation des données).
KaggleHub (pour télécharger les datasets depuis Kaggle).

Base de données :

PostgreSQL 13 (pour stocker les métadonnées d'Airflow).

Conteneurisation :

Docker (pour déployer Airflow et PostgreSQL).

Visualisation :

Power BI (pour l'analyse et les tableaux de bord).
